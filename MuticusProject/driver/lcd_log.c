...
//  LCD_DisplayStringLine(LCD_SCREEN_PIXEL_HEIGHT - cFont->Height, tmp);
//  LCD_DisplayStringLine(LCD_SCREEN_PIXEL_HEIGHT - cFont->Height, Status);
//  LCD_SetBackColor(Black);  
}
...
PUTCHAR_PROTOTYPE
{
  ...
  if(LCD_Lock == DISABLE)
  {
    ...
    if(( LCD_CacheBuffer_xptr < LCD_SCREEN_PIXEL_WIDTH /cFont->Width ) &&  ( ch != '\n'))
    {
    ...
    else 
    {
      ...
      for(idx = LCD_CacheBuffer_xptr ; idx < LCD_SCREEN_PIXEL_WIDTH /cFont->Width; idx++)
      {
