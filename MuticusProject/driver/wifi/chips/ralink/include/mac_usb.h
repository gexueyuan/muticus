...
typedef struct __TX_BUFFER{
    ...
    union{
        UCHAR            WirelessPacket[TX_BUFFER_NORMSIZE];        /*[TX_BUFFER_NORMSIZE]*/
        HEADER_802_11_WSUM NullFrame;
...
typedef struct __HTTX_BUFFER{
    union{
        UCHAR            WirelessPacket[MAX_TXBULK_SIZE];    /*[MAX_TXBULK_SIZE]*/
        HEADER_802_11_WSUM     NullFrame;
        PSPOLL_FRAME    PsPollPacket;
        RTS_FRAME        RTSFrame;
    }field;
...
/* The TX_BUFFER structure forms the transmitted USB packet to the device */
typedef struct __TX_NULL_BUFFER{
    union{
        UCHAR            WirelessPacket[/*TXINFO_SIZE + TXWI_SIZE + 48 + 4*/128];        /*[TX_BUFFER_NORMSIZE]*/
        HEADER_802_11_WSUM NullFrame;
        PSPOLL_FRAME    PsPollPacket;
        RTS_FRAME        RTSFrame;
    }field;
...
} TX_NULL_BUFFER, *PTX_NULL_BUFFER;

typedef struct _TX_NULL_CONTEXT
{
    PVOID            pAd;        /*Initialized in MiniportInitialize */
    PURB            pUrb;            /*Initialized in MiniportInitialize */
    //PIRP            pIrp;            /*used to cancel pending bulk out. */
                                    /*Initialized in MiniportInitialize */
    PTX_NULL_BUFFER    TransferBuffer;    /*Initialized in MiniportInitialize */
    ULONG            BulkOutSize;
    UCHAR            BulkOutPipeId;
    //UCHAR            SelfIdx;
    BOOLEAN            InUse;
...
}    TX_NULL_CONTEXT, *PTX_NULL_CONTEXT, **PPTX_NULL_CONTEXT;
